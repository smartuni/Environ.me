 ...
 * Changed by Nicolas Albers January 2015
 *
...
append_to_output(const unsigned char *data, size_t len) {
  ...
  if (con == NULL) 
  {
      ...
      fprintf(stderr, "database login\n"); // preconditon is to hav an Database with name "Environme"
  if (mysql_real_connect(con, "localhost", "root", "root", 
          "Environme", 0, NULL, 0) == NULL) 
  {
      fprintf(stderr, "database login no sucess\n");
      fprintf(stderr, "%s\n", mysql_error(con));
      mysql_close(con);
      exit(1);
  }else{
    fprintf(stderr, "database login sucess\n");
  }

//(char *)output_file.s contains the Table Name

/*

sprintf(str, "CREATE TABLE %s (Time_ms DOUBLE, Name TEXT);", (char *)output_file.s);
 fprintf(stderr, "SQL querry: %c", str);
...
strftime(tmbuf, sizeof tmbuf, "%Y-%m-%d %H:%M:%S", nowtm);
  ...
  //fprintf(stderr, "Time: %d milliseconds with %i Temperature\n", start.tv_sec, atoi(data));
  sprintf(str, "INSERT INTO temperatures VALUES( NULL, 1, '%s' ,%i);", tmbuf, atoi(data));
...
usage( const char *program, const char *version) {
  ...
  fprintf( stderr, "%s v%s -- a small CoAP implementation\n"
     ...
     "\t-o Database\t\toutput received data to this file (use '-' for STDOUT) this statement has to be followd by a \"Tablename\"\n"
