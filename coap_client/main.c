...
static int request_temperature(int argc, char **argv) {
    ...
    if (bind(client_socket, (struct sockaddr *)&client_addr,
	     client_addr_len) < 0) {
        client_socket = -1;
        puts("[main] ERROR: binding socket");
        return 1;
    }
    ...
    if ((recv_len = recvfrom(client_socket, client_buffer, buffer_size, 0,
			     ...
			     &server_addr_len)) < 0) {
	puts("[main] ERROR: on receive");
    } else if (recv_len == 0) {
	puts("[main] INFO:  peer did shut down");
    } else {
	// CoAP part
	if (0 != (rc = coap_parse(&inpkt, client_buffer, recv_len))) {
	    printf("[main] INFO:  bad packet rc=%d\n", rc);
	    return 1;
	}
	puts("[main] INFO:  redeived packet");
